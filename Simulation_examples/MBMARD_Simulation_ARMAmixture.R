...
# EXAMPLE of a mixture of AR(12) MA(4) and AR(1) processes
...
## The simulations are the mixture of AR(12) MA(4) and AR(1) as defined in Section 4

dimnodes<-7

S1<-arima.sim(n = size, list(ar = c(0,0,0,0.9,0,0,0,.7,0,0,0,-.63) ), sd = 1, n.start = 10000)
S1<-(S1-mean(S1))/sd(S1)+ rnorm(size, sd=.1)
S2<-arima.sim(n = size, list(ma = c(-.3,-.6,-.3,.6 ) ), sd = 1, n.start = 10000)
S2<-(S2-mean(S2))/sd(S2)+ rnorm(size, sd=.1)
S3<-arima.sim(n = size, list(ar = c(.8) ), sd = 1, n.start = 10000)
S3<-(S3-mean(S3))/sd(S3)+ rnorm(size, sd=.1)
S4<-arima.sim(n = size, list(ar = c(-.8) ), sd = 1, n.start = 10000)
